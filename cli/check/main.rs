...
fn main() {
	...
	let module =
		axia_wasm::deserialize_file(&input).expect("Input module deserialization failed");
					...
					match entry.external() {
						...
						elements::External::Memory(m) => {
							...
							let max = if let Some(max) = m.limits().maximum() {
								...
								fail("There is a limit on memory in Axia runtime, and this program does not limit memory");
							};

							if max > 16 {
								fail(&format!(
									"Axia runtime has 1Mb limit (16 pages) on max contract memory, this program speicifies {}",
									max
								));
							}
						},
						elements::External::Global(_) =>
							fail("Axia runtime does not provide any globals"),
						_ => continue,
					}
				}
