...
fn do_main() -> Result<(), Error> {
	...
	let module =
		axia_wasm::deserialize_file(&path).map_err(|e| Error::Decoding(e, path.to_string()))?;
	...
	if let Some(save_raw_path) = matches.value_of("save_raw") {
		axia_wasm::serialize_to_file(save_raw_path, module.clone()).map_err(Error::Encoding)?;
	}

	if let Some(ctor_module) = ctor_module {
		axia_wasm::serialize_to_file(&path, ctor_module).map_err(Error::Encoding)?;
	} else {
		axia_wasm::serialize_to_file(&path, module).map_err(Error::Encoding)?;
	}
