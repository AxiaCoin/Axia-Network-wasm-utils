...
fn main() {
	...
	// Loading module
	let module =
		axia_wasm::deserialize_file(&args[1]).expect("Module deserialization to succeed");

	let result = utils::inject_gas_counter(module, &utils::rules::Set::default(), "env")
		.expect("Failed to inject gas. Some forbidden opcodes?");

	axia_wasm::serialize_to_file(&args[2], result).expect("Module serialization to succeed")
}
