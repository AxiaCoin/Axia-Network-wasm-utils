...
impl Module {
	...
	/// Initialize module from axia-wasm `Module`.
			...
			match section {
				...
				elements::Section::Element(element_section) => {
					for element_segment in element_section.entries() {
						...
						// TODO: update axia-wasm and uncomment the above instead
						let init_expr = element_segment
							.offset()
							.as_ref()
							.expect("axia-wasm is compiled without bulk-memory operations")
							.code();
				...
				elements::Section::Data(data_section) => {
					for data_segment in data_section.entries() {
						// TODO: update axia-wasm and use the same logic as in
...
/// New module from axia-wasm `Module`
pub fn parse(wasm: &[u8]) -> Result<Module, Error> {
	Module::from_elements(&::axia_wasm::deserialize_buffer(wasm).map_err(Error::Format)?)
}

/// Generate axia-wasm `Module`
pub fn generate(f: &Module) -> Result<Vec<u8>, Error> {
	let pm = f.generate()?;
	::axia_wasm::serialize(pm).map_err(Error::Format)
}
